    ...
    <div class="post">
      <form @submit.prevent="checkForm($event)">
        ...
        <div class="lables-block">
          <div class="lables" 
            v-for="label in thisArticleLabels" 
            :key="label"  
          >      
            <div class="label-element"  >
              ...
              <div class="delete-label">
                <img class="delete-label-img" 
                  src="../assets/delete-label.png" 
                  alt="delete-label" 
                  @click="deleteThisLabel()"
                >
              </div>
            </div>
          </div>
          ...
          <div class="new-label">
            <p class="label-element-text">Create new label:</p>
              <input v-model="customLabel" 
                ...
                type="text" placeholder="New label title" 
              />
              <button class="new-label-btn" 
...
export default {
  ...
  methods: {
    ...
    addCustomLabel() {
      this.thisArticleLabels.push(this.customLabel)
      event.preventDefault()
    },
    ...
    async addNewPostVersion() {
      await (await fetch(`http://localhost:3000/posts/`, {
        ...
        body: JSON.stringify({
          ...
          body: this.postText,
          lablesList: this.thisArticleLabels,
        })
      })).json()
    }
  }
